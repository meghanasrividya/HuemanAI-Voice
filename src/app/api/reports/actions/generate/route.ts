...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[ACTIONS-REPORTS-GENERATE] Body sent to backend:", JSON.stringify(body));

    // Call the backend API for generate
    const response = await axios.post(
      "https://voice.huemanai.co.uk/api/reports/actions/generate",
      body,
      {
        headers,
    ...
    console.log("[ACTIONS-REPORTS-GENERATE] Backend status:", response.status);

    return NextResponse.json(response.data, {
      status: response.status,
    });
  } catch (error: any) {
    console.error("Error in actions generate API proxy:", error);
